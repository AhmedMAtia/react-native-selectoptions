...
class Select extends Component {
  ...
  _onPress() {
    const { optionListRef, children, onSelect, width, height,value } = this.props;
    if (!children.length) {
      return false;
    }
    ...
    return (
      <TouchableWithoutFeedback onPress={this._onPress.bind(this)}>
        <View ref={SELECT} style={[styles.container, style, dimensions ]}>
          <Option style={ styleOption } styleText={ styleText } value={this.props.value}>{this.state.title}</Option>
        </View>
      </TouchableWithoutFeedback>
    );
  }
